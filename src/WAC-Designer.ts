...
//  throwError,
    ...
    ValueIsNumber, ValueIsOrdinal,
    ValueIsText, ValueIsTextline,
    ValueIsPlainObject,
...
// @ts-ignore TS2307 typescript has problems importing "nanoid-dictionary"
  ...
  import {
    ...
    fromDocumentTo,
    WAC_Text, WAC_Geometry, WAC_Position, WAC_Location, WAC_URL,
    WAC_Category, WAC_Behavior,
    WAC_Visual, WAC_Applet, WAC_Page, WAC_Widget, WAC_ErrorReport,
    WAC_FontWeights, WAC_FontStyles,
    WAC_TextDecorationLines, WAC_TextDecorationStyles, WAC_TextAlignments,
    WAC_BackgroundModes, WAC_BorderStyles, WAC_Cursors, WAC_Overflows,
    WAC_ImageScalings, WAC_ImageAlignments, WAC_ReferrerPolicies,
    WAC_Orientations,
    WAC_TimePattern, WAC_DateTimePattern, WAC_DatePattern, WAC_WeekPattern,
      WAC_MonthPattern,
    WAC_PropertyDescriptor,
    ValueIsBehavior, ValueIsApplet, ValueIsPage, ValueIsWidget,
    ValueIsWidgetType, ValueIsErrorReport,
    allowPage,
    acceptableValue, ValueIsLineList, ValueIsNumberList,
    BehaviorIsIntrinsic,
    GestureRecognizer,
...
/**** appendStyle ****/
  ...
  function appendStyle (Style:WAC_Text):void {
...
/**** AssetsBase, IconFolder ****/
  ...
  let AssetsBase:WAC_URL = 'https://rozek.github.io/webapp-crafter/'
  let IconFolder:WAC_URL = AssetsBase + 'icons/'
...
//------------------------------------------------------------------------------
  ...
  const DesignerState:Indexable = {
    ...
    SettingsDialog: {                        // only visible if Designer is open
      Title:'WAC Settings', View:undefined,
    ...
    CodeAssistant: {                         // only visible if Designer is open
      Title:'WAC Code Assistant', View:undefined,
      x:NaN, y:NaN, Width:320, Height:240,
      minWidth:320, minHeight:240,
      Scope:'Applet',
      ReportToShow:undefined, // workaround for strange closure problem
    ...
    SearchDialog: {                          // only visible if Designer is open
      Title:'Search within WAC', View:undefined,
      x:NaN, y:NaN, Width:320, Height:240,
      minWidth:320, minHeight:240,
    },
...
/**** selectPages ****/
  ...
  function selectPages (PageList:WAC_Page[]):void {
...
/**** sortedPageSelection ****/
  ...
  function sortedPageSelection ():WAC_Page[] {
    const IndexSet:WAC_Page[] = []
      DesignerState.selectedPages.forEach((Page:WAC_Page) => IndexSet[Page.Index] = Page)
...
/**** selectWidgets ****/
  ...
  function selectWidgets (
    SelectionA:WAC_Widget[], SelectionB:WAC_Widget[] = []
  ):void {
    const newSelection:WAC_Widget[] = SelectionA.slice()
      SelectionB.forEach((Widget:WAC_Widget) => {
...
/**** sortedWidgetSelection ****/
  ...
  function sortedWidgetSelection ():WAC_Widget[] {
    const IndexSet:WAC_Widget[] = []
      DesignerState.selectedWidgets.forEach((Widget:WAC_Widget) => IndexSet[Widget.Index] = Widget)
    const IndexList = Object.keys(IndexSet).map(Number).sort()
    return IndexList.map((Index:number) => IndexSet[Index])
  }
...
/**** WidgetIsSelected ****/
  ...
  function WidgetIsSelected (Widget:WAC_Widget):boolean {
      ...
      openDesigner()                                  // if not yet already done
        ...
        switch (true) {
          ...
          case ValueIsPage(Sufferer):
            visitPage(Sufferer as WAC_Page)
            DesignerState.ScriptEditor.Scope = 'visitedPage'
            break
          case ValueIsWidget(Sufferer):
            visitPage((Sufferer as WAC_Widget).Page)
            selectWidgets([Sufferer as WAC_Widget])
...
/**** looksLikeApplet ****/
  ...
  function looksLikeApplet (Serialization:Serializable):boolean {
    ...
    )                                // a proper "PageList" identifies an applet
  }
...
/**** looksLikePage ****/
  ...
  function looksLikePage (Serialization:Serializable):boolean {
    ...
    )                                 // a proper "WidgetList" identifies a page
  }
...
/**** looksLikeWidget ****/
  ...
  function looksLikeWidget (Serialization:Serializable):boolean {
    return (
      ...
      (Serialization.PageList   == null) &&
      (Serialization.WidgetList == null)
    )            // neither "PageList" nor "WidgetList"? that should be a widget
  }
...
/**** generateEmbeddableApplet - with integrated script ****/
  ...
  function generateEmbeddableApplet ():void {
    ...
    const AppletName    = Applet.Name || 'WAC-Applet'
    const Serialization = JSON.stringify(Applet.Serialization)
    const AppletSource  = `${'<'}script type="wac/applet">${Serialization}${'<'}/script>`
...
<html lang="en" charset="utf-8" style="width:100%">
 <head>
  ...
  ${withDesigner ? `${'<'}script src="https://rozek.github.io/webapp-crafter/js/WAC-Designer.esm.js" type="module">${'<'}/script>` : ''}
...
/**** visitPage ****/
  ...
  function visitPage (Page:WAC_Page|undefined):void {
...
//----------------------------------------------------------------------------//
  ...
  class WAD_BehaviorDeserializationOperation extends WAD_Operation {
    private _Category:WAC_Category
    private _Behavior:WAC_Behavior
    private _Script:WAC_Text
...
//----------------------------------------------------------------------------//
  ...
  class WAD_BehaviorScriptApplicationOperation extends WAD_Operation {
    private _Category:WAC_Category
    private _Behavior:WAC_Behavior
    private _oldScript:WAC_Text
    private _newScript:WAC_Text

  /**** constructor ****/

    public constructor (Category:WAC_Category, Behavior:WAC_Behavior) {
      super()
...
//----------------------------------------------------------------------------//
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      ...
      const newPages:WAC_Page[] = this._newPages = []
  ...
  /**** undo ****/
    ...
    public undo ():void {
      const Applet = DesignerState.Applet
      this._newPages.forEach((Page:WAC_Page) => {
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (
      Pages:WAC_Page[],  PropertyName:string, PropertyValue:any
    ) {
      super()

      this._Pages        = Pages.slice()
      this._PropertyName = PropertyName
      this._oldValues    = Pages.map((Page:WAC_Page) => Page[PropertyName])
      this._newValue     = PropertyValue
    }
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      this._Pages.forEach((Page:WAC_Page) => {
  ...
  /**** undo ****/
    ...
    public undo ():void {
      this._Pages.forEach((Page:WAC_Page, i:number) => {
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (Pages:WAC_Page[]) {
      super()

      this._Pages      = Pages.slice()
      this._oldScripts = Pages.map((Page:WAC_Page) => Page.activeScript)
      this._newScripts = Pages.map((Page:WAC_Page) => Page.pendingScript)
    }
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      this._Pages.forEach((Page:WAC_Page) => Page.applyPendingScript())
    }
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (Pages:WAC_Page[], newIndices:number[]) {
      super()

      this._Pages      = Pages.slice()
      this._oldIndices = Pages.map((Page:WAC_Page) => Page.Index)
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (Pages:WAC_Page[]) {
      ...
      const IndexSet:WAC_Page[] = []
        Pages.forEach((Page:WAC_Page) => IndexSet[Page.Index] = Page)
      this._Indices        = Object.keys(IndexSet).map(Number).sort()
      this._Pages          = this._Indices.map((Index:number) => IndexSet[Index])
      this._Serializations = this._Pages.map((Page:WAC_Page) => Page.Serialization)
    }
  ...
  /**** undo ****/
    ...
    public undo ():void {
      ...
      const newPages:WAC_Page[] = this._Pages = []
        this._Serializations.forEach((Serialization:Indexable, i:number) => {
          const newPage = Applet.PageDeserializedAt(
...
//----------------------------------------------------------------------------//
  ...
  class WAD_WidgetDeserializationOperation extends WAD_Operation {
    private _Page:WAC_Page
    private _Serializations:Indexable[]
    private _StartIndex:number
    private _newWidgets:WAC_Widget[] = []

  /**** constructor ****/

    public constructor (
      Serializations:Indexable[], Page:WAC_Page, StartIndex:number
    ) {
      super()
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      const newWidgets:WAC_Widget[] = this._newWidgets = []
        this._Serializations.forEach((Serialization:Indexable, i:number) => {
  ...
  /**** undo ****/
    ...
    public undo ():void {
      this._newWidgets.forEach((Widget:WAC_Widget) => {
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (
      Widgets:WAC_Widget[],  PropertyName:string, PropertyValues:any[]
    ) {
      super()

      this._Widgets      = Widgets.slice()
      this._PropertyName = PropertyName
      this._oldValues    = Widgets.map((Widget:WAC_Widget) => Widget[PropertyName])
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      this._Widgets.forEach((Widget:WAC_Widget, i:number) => {
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (Widgets:WAC_Widget[]) {
      super()

      this._Widgets    = Widgets.slice()
      this._oldScripts = Widgets.map((Widget:WAC_Widget) => Widget.activeScript)
      this._newScripts = Widgets.map((Widget:WAC_Widget) => Widget.pendingScript)
    }
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      this._Widgets.forEach((Widget:WAC_Widget) => Widget.applyPendingScript())
    }
...
//----------------------------------------------------------------------------//
  ...
  class WAD_WidgetShapeOperation extends WAD_Operation {
    private _Page:WAC_Page
    private _Widgets:WAC_Widget[]
    private _oldGeometries:WAC_Geometry[]
    private _newGeometries:WAC_Geometry[]

  /**** constructor ****/

    public constructor (
      Widgets:WAC_Widget[], Geometries:WAC_Geometry[]
    ) {
      super()

      this._Page = Widgets[0].Page
        if (Widgets.some((Widget:WAC_Widget) => Widget.Page !== this._Page)) throwError(
          'InvalidArgument: the given widgets do not all belong to the same page'
        )
      this._Widgets       = Widgets.slice()
      this._oldGeometries = Widgets.map((Widget:WAC_Widget) => Widget.Geometry)
  ...
  /**** isIrrelevant ****/
    ...
    public get isIrrelevant ():boolean {
      return this._oldGeometries.every(
        (Geometry:WAC_Geometry, i:number) => ValuesAreEqual(this._newGeometries[i],Geometry)
      )
    }
    public set isIrrelevant (_:boolean) { throwReadOnlyError('isIrrelevant') }
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (Widgets:WAC_Widget[], newIndices:number[]) {
      super()
      ...
      this._Widgets    = Widgets.slice()
      this._oldIndices = Widgets.map((Widget:WAC_Widget) => Widget.Index)
      this._newIndices = newIndices.slice()
    }
...
//----------------------------------------------------------------------------//
  ...
  /**** constructor ****/
    ...
    public constructor (Widgets:WAC_Widget[]) {
      ...
      const IndexSet:WAC_Widget[] = []
        Widgets.forEach((Widget:WAC_Widget) => IndexSet[Widget.Index] = Widget)
      this._Indices        = Object.keys(IndexSet).map(Number).sort()
      this._Widgets        = this._Indices.map((Index:number) => IndexSet[Index])
      this._Serializations = this._Widgets.map((Widget:WAC_Widget) => Widget.Serialization)
    }
  ...
  /**** doNow ****/
    ...
    public doNow ():void {
      this._Widgets.forEach((Widget:WAC_Widget) => {
        this._Page.destroyWidget(Widget)
      })
    }
  ...
  /**** undo ****/
    ...
    public undo ():void {
      const newWidgets:WAC_Widget[] = this._Widgets = []
        this._Serializations.forEach((Serialization:Indexable, i:number) => {
          const newWidget = this._Page.WidgetDeserializedAt(
            Serialization, this._Indices[i]
          )
...
/**** doCreatePage ****/
  ...
  function doCreatePage (Behavior:string):void {
    ...
    const InsertionIndex = (
      ...
      ? Applet.PageCount
      : Math.max(selectedPages.map((Page:WAC_Page) => Page.Index))+1
    )
...
/**** doDuplicateSelectedPages ****/
  ...
  function doDuplicateSelectedPages ():void {
    ...
    doOperation(new WAD_PageDeserializationOperation(
      selectedPages.map((Page:WAC_Page) => Page.Serialization), InsertionIndex
    ))
  }
...
/**** doDuplicateSelectedWidgets ****/
  ...
  function doDuplicateSelectedWidgets ():void {
    ...
    doOperation(new WAD_WidgetDeserializationOperation(
      selectedWidgets.map((Widget:WAC_Widget) => Widget.Serialization),
      DesignerState.Applet.visitedPage,0
    ))
  }
  ...
  function doConfigureSelectedWidgets (Property:string, Value:any):void {
    ...
    let ValuesToSet:any|any[]
      switch (Property) {
        case 'Anchors_0':
          Property    = 'Anchors'
          ValuesToSet = selectedWidgets.map((Widget:WAC_Widget) => {
...
/**** doChangeGeometriesBy ****/
  ...
  function doChangeGeometriesBy (
    WidgetList:WAC_Widget[], Mode:string, dx:number,dy:number,
    initialGeometries:WAC_Geometry[], withSnapToGrid:boolean = true
  ):void {
...
console.error(Signal)
    ...
    window.alert('file does not contain valid WAC serializations')
  }
  ...
  function doExport (Scope:string):void {
    ...
    switch (Scope) {
      ...
      case 'Applet':
        ...
        suggestedFileName = (Applet.Name || 'WAC-Applet') + '.json'
        break
      case 'active Page':
        Serialization     = Applet.visitedPage.Serialization
        suggestedFileName = (Applet.visitedPage.Name || 'WAC-Page') + '.json'
        break
      case 'selected Pages':
        const Pages = sortedPageSelection()
// @ts-ignore TS2322 allow assignment
        Serialization     = Pages.map((Page:WAC_Page) => Page.Serialization)
        suggestedFileName = (Pages[0]?.Name || 'WAC-Page') + '.json'
        break
      case 'selected Widgets':
        const Widgets = DesignerState.selectedWidgets
        Serialization     = Widgets.map((Widget:WAC_Widget) => Widget.Serialization)
        suggestedFileName = (Widgets[0]?.Name || 'WAC-Widgets') + '.json'
        break
      case 'Applet Design':
        Serialization = Applet.Serialization
          delete Serialization.activeScript
          delete Serialization.pendingScript
        suggestedFileName = (DesignerState.Applet.Name || 'WAC-Applet') + '.json'
        break
      case 'Applet Script':
        Serialization     = Applet.activeScript || ''
        suggestedFileName = (Applet.Name || 'WAC-Applet') + '.js'
        break
      default:
...
/**** doCreateScreenshot ****/
  ...
  function doCreateScreenshot ():void {
    ...
    DesignerState.DesignerDisabled = true; WAC_rerender()
...
// @ts-ignore TS18047 "Context" is not null
        ...
        DesignerState.DesignerDisabled = false; WAC_rerender()

        const Name = (
          VisualToRender === Applet
          ? Applet.visitedBoard?.Name || Applet.Name || 'WAC-Applet'
          : VisualToRender.Name || 'WAC-Screenshot.png'
        )
...
/**** selectedPagesMayBeShiftedUp ****/
  ...
  function selectedPagesMayBeShiftedUp ():boolean {
    const selectedPages = sortedPageSelection()
    return selectedPages.some((Page:WAC_Page,i:number) => Page.Index > i)
  }
...
/**** selectedPagesMayBeShiftedDown ****/
  ...
  function selectedPagesMayBeShiftedDown ():boolean {
    ...
    return selectedPages.some((Page:WAC_Page,i:number) => Page.Index < StartIndex+i)
  }
...
/**** selectedWidgetsMayBeShiftedUp ****/
  ...
  function selectedWidgetsMayBeShiftedUp ():boolean {
    const selectedWidgets = sortedWidgetSelection()
    return selectedWidgets.some((Widget:WAC_Widget,i:number) => Widget.Index > i)
  }
...
/**** selectedWidgetsMayBeShiftedDown ****/
  ...
  function selectedWidgetsMayBeShiftedDown ():boolean {
    ...
    return selectedWidgets.some((Widget:WAC_Widget,i:number) => Widget.Index < StartIndex+i)
  }
...
/**** shrinkApplet ****/
  ...
  function shrinkApplet ():void {
    ...
    let minY:number = Infinity, maxY:number = 0
      Applet.PageList.forEach((Page:WAC_Page) => {
...
/**** resizeAppletTo ****/
  ...
  function resizeAppletTo (
    Applet:WAC_Applet, Width:number,Height:number,
...
//------------------------------------------------------------------------------
  ...
  function WAD_DesignerLayer (PropSet:Indexable):any {
    ...
    AssetsBase = PropSet.AssetsBase as WAC_URL
    IconFolder = AssetsBase + 'icons/'

  /**** if need be: initialize VisitHistory ****/

    const Applet = DesignerState.Applet = PropSet.Applet as WAC_Applet
...
/**** WAD_BehaviorBrowserPane ****/
  ...
  /**** handle list item rendering and selection ****/
    ...
    const BehaviorListItemRenderer = useCallback(
      (GroupName:string, Behavior:WAC_Behavior, Index:number, selected:boolean) => {
     ...
     <${WAD_vertically} style="width:100%; height:100%; padding:4px">
      ...
      <${WAD_horizontally} style="padding-top:4px; padding-bottom:4px">
        ...
        <${WAD_TextlineInput} Placeholder="(behavior name)" style="flex:1 0 auto"
          ...
          onInput=${(Event:Indexable) => { Inspector.newBehaviorName = Event.target.value; WAC_rerender() }}
        />
          <div style="width:8px"/>
...
/**** WAD_AppletConfigurationPane ****/
  ...
  /**** prepare behavior-specific properties ****/
    ...
    if (configurableProperties.length === 0) {
      ...
      PropertyEditors = configurableProperties.map((Descriptor:WAC_PropertyDescriptor) => html`
  ...
  /**** handle script input ****/
    ...
    const setPendingScriptTo = useCallback((newScript:WAC_Text) => {
      ...
      " ref=${scrollablePane} scrollTop=${ScrollPosition} onScroll=${updateScrollPosition}>
        ...
        >
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${Applet.FontWeight} Options=${WAC_FontWeights}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${Applet.FontStyle} Options=${WAC_FontStyles}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${Applet.TextAlignment} Options=${WAC_TextAlignments}
        ...
        >
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${Applet.BackgroundTexture?.Mode} Options=${WAC_BackgroundModes}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  Applet.BackgroundTexture || { isActive:false, ImageURL:'', Mode:'tile', xOffset:0,yOffset:0 }
                )
...
/**** WAD_PageBrowserPane ****/
  ...
  /**** handle list item rendering and selection ****/
    ...
    const PageListItemRenderer = useCallback((Page:WAC_Page, Index:number, selected:boolean) => {
      let Style = ''
    ...
    const selectedPageIndices = selectedPages.map((Page:WAC_Page) => Page.Index)
     ...
     <${WAD_vertically} style="width:100%; height:100%; padding:4px">
      ...
      <${WAD_horizontally} style="padding-top:4px">
        ...
        <${WAD_TextlineInput} Placeholder="(page name)" style="flex:1 0 auto"
          enabled=${selectedPages.length > 0}
          Value=${commonValueOf(selectedPages.map((Page:WAC_Page) => Page.Name))}
      ...
      " ref=${scrollablePane} scrollTop=${ScrollPosition} onScroll=${updateScrollPosition}>
        ...
        >
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${visitedPage.FontWeight} Options=${WAC_FontWeights}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${visitedPage.FontStyle} Options=${WAC_FontStyles}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${visitedPage.TextAlignment} Options=${WAC_TextAlignments}
        ...
        >
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Gap}/>
            <${WAD_DropDown}
              Value=${visitedPage.BackgroundTexture?.Mode} Options=${WAC_BackgroundModes}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  visitedPage.BackgroundTexture || { isActive:false, ImageURL:'', Mode:'tile', xOffset:0,yOffset:0 }
                )
...
/**** WAD_WidgetBrowserPane ****/
  ...
  /**** handle list item rendering and selection ****/
    ...
    const WidgetListItemRenderer = useCallback((Widget:WAC_Widget, Index:number, selected:boolean) => {
    ...
    const selectedWidgetIndices = selectedWidgets.map((Widget:WAC_Widget) => Widget.Index)
     ...
     <${WAD_vertically} style="width:100%; height:100%; padding:4px">
      ...
      <${WAD_horizontally} style="padding-top:4px">
        ...
        <${WAD_TextlineInput} Placeholder="(widget name)" style="flex:1 0 auto"
          enabled=${selectedWidgets.length > 0}
          Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Name))}
...
/**** WAD_WidgetConfigurationPane ****/
  ...
  /**** handle value input ****/
    ...
    let ValueToEdit = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Value))
      switch (true) {
  ...
  /**** prepare behavior-specific properties ****/
    ...
    let configurableProperties = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.configurableProperties))
    ...
    if (configurableProperties.length === 0) {
      ...
      PropertyEditors = configurableProperties.map((Descriptor:WAC_PropertyDescriptor) => html`
        <${WAD_PropertyConfigurator} Descriptor=${Descriptor}
          ...
          Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget[Descriptor.Name]))}
  ...
  /**** handle script input ****/
    ...
    const activeScript  = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.activeScript))
    const pendingScript = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.pendingScript))
    const ErrorReport   = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.ErrorReport))
    const ScriptError   = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.ScriptError))
    ...
    return html`<div class="WAD InspectorPane">
     <${WAD_vertically} style="width:100%; height:100%; padding:4px">
      ...
      <${WAD_horizontally} style="padding-top:4px">
        ...
        <${WAD_BehaviorDropDown}
          enabled=${false}
          Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Behavior))}
          groupedOptionList=${Applet.groupedBehaviorListOfCategory('widget')}
        />
      </>
      ...
      <${WAD_TextInput} Placeholder="(enter synopsis)" style="
        padding-top:4px; min-height:60px;
      " enabled=${selectedWidgets.length > 0}
        Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Synopsis))}
      ...
      " ref=${scrollablePane} scrollTop=${ScrollPosition} onScroll=${updateScrollPosition}>
        ...
        >
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Visibility))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Lock))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Opacity))}
          ...
          <${WAD_horizontally}>
            <${WAD_Label}>Overflows</>
            <${WAD_Gap}/>
            <${WAD_DropDown} Options=${WAC_Overflows}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Overflows),0)}
              onInput=${(Event:Indexable) => {
                const [ Overflow_0,Overflow_1 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Overflows || ['visible','visible']))
                )
            ...
            <${WAD_DropDown} Options=${WAC_Overflows}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Overflows),1)}
              onInput=${(Event:Indexable) => {
                const [ Overflow_0,Overflow_1 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Overflows || ['visible','visible']))
                )
          ...
          <${WAD_horizontally} style="padding-top:4px">
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Enabling))}
        ...
        >
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.x))}
              onInput=${(Event:Indexable) => doConfigureSelectedWidgets('x',parseFloat(Event.target.value))}
            />
              <div style="width:20px; padding-top:4px; text-align:center">,</div>
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.y))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Width))}
              onInput=${(Event:Indexable) => doConfigureSelectedWidgets('Width',parseFloat(Event.target.value))}
            />
              <div style="width:20px; padding-top:4px; text-align:center">x</div>
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Height))}
          ...
          <${WAD_horizontally} style="padding-top:4px">
            ...
            <${WAD_DropDown} style="width:104px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Anchors[0]))}
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Offsets[0]))}
              onInput=${(Event:Indexable) => doConfigureSelectedWidgets('Offsets_0',parseFloat(Event.target.value))}
            />
              <div style="width:20px; padding-top:4px; text-align:center">,</div>
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Offsets[1]))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown} style="width:104px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Anchors[1]))}
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Offsets[2]))}
              onInput=${(Event:Indexable) => doConfigureSelectedWidgets('Offsets_2',parseFloat(Event.target.value))}
            />
              <div style="width:20px; padding-top:4px; text-align:center">,</div>
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Offsets[3]))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.minWidth))}
              onInput=${(Event:Indexable) => doConfigureSelectedWidgets('minWidth',parseFloat(Event.target.value))}
            />
              <div style="width:20px; padding-top:4px; text-align:center">x</div>
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.minHeight))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.maxWidth))}
              onInput=${(Event:Indexable) => doConfigureSelectedWidgets('maxWidth',parseFloat(Event.target.value))}
            />
              <div style="width:20px; padding-top:4px; text-align:center">x</div>
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.maxHeight))}
        ...
        >
          <${WAD_horizontally}>
            ...
            <${WAD_TextlineInput} style="flex:1 0 auto"
              ...
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.FontFamily))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.FontSize))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.FontWeight))}
              Options=${WAC_FontWeights}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.FontStyle))}
              Options=${WAC_FontStyles}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Color))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration),'isActive')}
              onInput=${(Event:Indexable) => {
                const { isActive, Line, Color, Style, Thickness } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration),'Line')}
              Options=${WAC_TextDecorationLines}
              onInput=${(Event:Indexable) => {
                const { isActive, Line, Color, Style, Thickness } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration
                    || { isActive:false, Line:'none', Color:'black', Style:'solid', Thickness:1 }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration),'Color')}
              onInput=${(Event:Indexable) => {
                const { isActive, Line, Color, Style, Thickness } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration
                    || { isActive:false, Line:'none', Color:'black', Style:'solid', Thickness:1 }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration),'Style')}
              Options=${WAC_TextDecorationStyles}
              onInput=${(Event:Indexable) => {
                const { isActive, Line, Color, Style, Thickness } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration
                    || { isActive:false, Line:'none', Color:'black', Style:'solid', Thickness:1 }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} readonly style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration),'Thickness')}
              onInput=${(Event:Indexable) => {
                const { isActive, Line, Color, Style, Thickness } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextDecoration
                    || { isActive:false, Line:'none', Color:'black', Style:'solid', Thickness:1 }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow),'isActive')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow),'Color')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5, Color:'black' }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} readonly style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow),'xOffset')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5, Color:'black' }
            ...
            <${WAD_IntegerInput} readonly style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow),'yOffset')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow
                   || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5, Color:'black' }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} readonly style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow),'BlurRadius')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextShadow
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5, Color:'black' }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.TextAlignment))}
              Options=${WAC_TextAlignments}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.LineHeight))}
              Minimum=${0} Maximum=${1000}
        ...
        >
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.hasBackground))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundColor))}
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture),'isActive')}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture))
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture),'Mode')}
              Options=${WAC_BackgroundModes}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture))
                    || { isActive:false, ImageURL:'', Mode:'tile', xOffset:0,yOffset:0 }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_TextlineInput} Type="url" style="flex:1 0 auto"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture),'ImageURL')}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture))
                    || { isActive:false, ImageURL:'', Mode:'tile', xOffset:0,yOffset:0 }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} readonly style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture),'xOffset')}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture))
                    || { isActive:false, ImageURL:'', Mode:'tile', xOffset:0,yOffset:0 }
            ...
            <${WAD_IntegerInput} readonly style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture),'yOffset')}
              onInput=${(Event:Indexable) => {
                const { isActive, ImageURL, Mode, xOffset,yOffset } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BackgroundTexture))
                    || { isActive:false, ImageURL:'', Mode:'tile', xOffset:0,yOffset:0 }
        ...
        >
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles),0)}
              Options=${WAC_BorderStyles}
              onInput=${(Event:Indexable) => {
                const [ Style_0,Style_1,Style_2,Style_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles || ['none','none','none','none']))
                )
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths),0)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Width_0,Width_1,Width_2,Width_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths || [0,0,0,0]))
                )
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors),0)}
              onInput=${(Event:Indexable) => {
                const [ Color_0,Color_1,Color_2,Color_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors || ['black','black','black','black']))
                )
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles),1)}
              Options=${WAC_BorderStyles}
              onInput=${(Event:Indexable) => {
                const [ Style_0,Style_1,Style_2,Style_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles || ['none','none','none','none']))
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths),1)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Width_0,Width_1,Width_2,Width_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths || [0,0,0,0]))
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors),1)}
              onInput=${(Event:Indexable) => {
                const [ Color_0,Color_1,Color_2,Color_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors || ['black','black','black','black']))
                )
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles),2)}
              Options=${WAC_BorderStyles}
              onInput=${(Event:Indexable) => {
                const [ Style_0,Style_1,Style_2,Style_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles || ['none','none','none','none']))
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths),2)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Width_0,Width_1,Width_2,Width_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths || [0,0,0,0]))
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors),2)}
              onInput=${(Event:Indexable) => {
                const [ Color_0,Color_1,Color_2,Color_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors || ['black','black','black','black']))
                )
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles),3)}
              Options=${WAC_BorderStyles}
              onInput=${(Event:Indexable) => {
                const [ Style_0,Style_1,Style_2,Style_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderStyles || ['none','none','none','none']))
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths),3)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Width_0,Width_1,Width_2,Width_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderWidths || [0,0,0,0]))
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors),3)}
              onInput=${(Event:Indexable) => {
                const [ Color_0,Color_1,Color_2,Color_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderColors || ['black','black','black','black']))
                )
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii),0)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Radius_0,Radius_1,Radius_2,Radius_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii || [0,0,0,0]))
                )
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii),1)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Radius_0,Radius_1,Radius_2,Radius_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii || [0,0,0,0]))
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii),3)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Radius_0,Radius_1,Radius_2,Radius_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii || [0,0,0,0]))
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii),2)}
              Minimum=${0}
              onInput=${(Event:Indexable) => {
                const [ Radius_0,Radius_1,Radius_2,Radius_3 ] = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BorderRadii || [0,0,0,0]))
        ...
        >
          <${WAD_horizontally}>
            ...
            <${WAD_Checkbox}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow),'isActive')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius,SpreadRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow))
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_ColorInput}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow),'Color')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius,SpreadRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow))
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5,SpreadRadius:0, Color:'black' }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow),'xOffset')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius,SpreadRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow))
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5,SpreadRadius:0, Color:'black' }
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow),'yOffset')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius,SpreadRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow))
                   || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5,SpreadRadius:0, Color:'black' }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow),'BlurRadius')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius,SpreadRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow))
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5,SpreadRadius:0, Color:'black' }
          ...
          <${WAD_horizontally}>
            ...
            <${WAD_IntegerInput} style="width:60px"
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueItemOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow),'SpreadRadius')}
              onInput=${(Event:Indexable) => {
                const { isActive, xOffset,yOffset, BlurRadius,SpreadRadius, Color } = (
                  commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.BoxShadow))
                    || { isActive:false, xOffset:0,yOffset:0, BlurRadius:5,SpreadRadius:0, Color:'black' }
        ...
        >
          <${WAD_horizontally}>
            ...
            <${WAD_DropDown}
              enabled=${selectedWidgets.length > 0}
              Value=${commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Cursor))}
              Options=${WAC_Cursors}
...
//------------------------------------------------------------------------------
  ...
  function WAD_BehaviorEditor () {
    ...
    let activeScript:WAC_Text|undefined, pendingScript:WAC_Text|undefined
    let ErrorReport:WAC_ErrorReport|undefined, ScriptError:WAC_ErrorReport|undefined
...
//------------------------------------------------------------------------------
  ...
  function WAD_SynopsisEditor () {
    ...
    let Synopsis:WAC_Text|undefined
    ...
    switch (Scope) {
      ...
      case 'selectedWidgets':
        const { selectedWidgets } = DesignerState
        Synopsis = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.Synopsis))
        break
    }
    ...
    const setSynopsisTo = useCallback((newSynopsis:WAC_Text) => {
...
//------------------------------------------------------------------------------
  ...
  function WAD_ValueEditor () {
    ...
    let EditorType:any = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => {
      return Widget.configurableProperty('Value')?.EditorType
    }))
    ...
    let ValueType:string|undefined
      switch (EditorType) {
        ...
        case 'color-input':
        case 'drop-down':
          ValueType = 'textline'; break
        case 'number-input':
        case 'integer-input':
        case 'slider':
          ValueType = 'number'; break
        case 'text-input':
        case 'html-input':
        case 'css-input':
        case 'javascript-input':
        case 'json-input':
          ValueType = 'text'; break
        case 'linelist-input':
          ValueType = 'linelist'; break
        case 'numberlist-input':
          ValueType = 'numberlist'; break
      }
...
//------------------------------------------------------------------------------
  ...
  function WAD_ScriptEditor () {
    ...
    switch (Scope) {
      ...
      case 'selectedWidgets':
        const { selectedWidgets } = DesignerState
        activeScript  = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.activeScript))
        pendingScript = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.pendingScript))
        ErrorReport   = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.ErrorReport))
        ScriptError   = commonValueOf(selectedWidgets.map((Widget:WAC_Widget) => Widget.ScriptError))
        break
    }
...
/**** onCoverClick ****/
  ...
  function onCoverClick (
    ...
    const Widget = LayouterState.pointedWidget as WAC_Widget
    ...
    if (Event.shiftKey || Event.metaKey) {     // additive/subtractive selection
      ...
      if (WidgetIsSelected(Widget)) {
        selectedWidgets = selectedWidgets.filter(
          (selectedWidget:WAC_Widget) => (selectedWidget !== Widget)
        )
      } else {
...
/**** Widget Drag/Select Recognizer ****/
  ...
  LayouterState.CoverRecognizer = GestureRecognizer({
    ...
    onDragStart:(dx:number,dy:number, StartX:number,StartY:number, Event:Indexable) => {
      ...
      if (! WidgetIsSelected(LayouterState.pointedWidget as WAC_Widget)) {
        if (Event.shiftKey || Event.metaKey) { // additive/subtractive selection
          selectWidgets([pointedWidget as WAC_Widget],DesignerState.selectedWidgets)
        } else {
          selectWidgets([pointedWidget as WAC_Widget])
        }
      }

      LayouterState.shapedWidgets     = sortedWidgetSelection()
      LayouterState.initialGeometries = LayouterState.shapedWidgets.map(
        (Widget:WAC_Widget) => Widget.Geometry
      )
      doChangeGeometriesBy(
...
/**** ShapeHandle Drag Recognizer ****/
  ...
  function GeometryOfLasso ():WAC_Geometry {
    const { x:x0,y:y0 } = LayouterState.LassoStart as WAC_Position
    const { x:x1,y:y1 } = LayouterState.LassoEnd || LayouterState.LassoStart as WAC_Position
...
/**** WidgetsCaughtByLasso ****/
  ...
  function WidgetsCaughtByLasso ():WAC_Widget[] {
    ...
    if (LayouterState.LassoMode === 'touch') {
      return DesignerState.Applet.visitedPage.WidgetList.filter((Widget:WAC_Widget) => {
...
/**** dragLassoTo ****/
  ...
  function dragLassoTo (
    x:WAC_Location,y:WAC_Location, additiveSelection:boolean
  ):void {
  ...
  function abortDraggingAndShaping ():void {
    if (LayouterState.shapedWidgets.length > 0) {
      doOperation(new WAD_WidgetShapeOperation(
        LayouterState.shapedWidgets,LayouterState.initialGeometries as WAC_Geometry[]
      ))
    }
    finishDraggingAndShaping()
  }
...
//------------------------------------------------------------------------------
  ...
  /**** handleCoverEvent ****/
    ...
    function handleCoverEvent (Event:PointerEvent, Widget:WAC_Widget):void {
...
// @ts-ignore TS18046 allow "window.showOpenFilePicker"
        ...
        case 'ArrowLeft':
          ...
          if (Event.altKey) {                                            // size
            doChangeGeometriesBy(DesignerState.selectedWidgets,'e',
              ...
              DesignerState.selectedWidgets.map((Widget:WAC_Widget) => Widget.Geometry),
              false
            )
          } else {                                                       // move
  ...
  /**** actual rendering ****/
    ...
    return html`<div class="WAD LayouterLayer" ref=${DOMElement} tabindex="0"
      ...
      onKeyDown=${onKeyDown}
    >
      ${WidgetList.toReversed().map((Widget:WAC_Widget) => {
      ...
      ${(selectedWidgets.length > 0)
        ? selectedWidgets.filter(
            (Widget:WAC_Widget) => Widget.isVisible
          ).toReversed().map((Widget:WAC_Widget) => {
            const WidgetId = IdOfWidget(Widget)
...
//------------------------------------------------------------------------------
  ...
  function horizontalGuides ():any {
    ...
    WidgetList.filter(
      (Widget:WAC_Widget) => ! WidgetIsSelected(Widget)
    ).forEach((Widget:WAC_Widget) => {
    ...
    selectedWidgets.forEach((Widget:WAC_Widget) => {
      const { y,Height } = Widget.Geometry
        const yt = Math.round(y)
        const ym = Math.round(y+Height/2)
        const yb = Math.round(y+Height)
...
/**** vertical Guides ****/
  ...
  const IdForWidget:WeakMap<WAC_Widget,string> = new WeakMap()

  function IdOfWidget (Widget:WAC_Widget):string {
...
/**** inform WAC about this designer ****/
  ...
  localforage.ready(async function () {
    ...
    console.log('starting WebApp Crafter Designer...')
//      WAD_DesignerLayer.showErrorReport = showErrorReport
      useDesigner(WAD_DesignerLayer)
    console.log('WebApp Crafter Designer is operational')
  })
